...
checkpoint_file = "llama3_translation_checkpoint_pivot_fi.pkl"
...
try:
    ...
    with tqdm(total=total_items, desc="Hebrew → English → Finnish") as pbar:
        for idx, (key, value) in enumerate(items):
            hebrew_text = value[0]
            
            try:
                # Pivot translation: Hebrew -> English -> Finnish
                english_text = translate_text(hebrew_text, "Hebrew", "English")
                finnish_text = translate_text(english_text, "English", "Finnish")
    ...
    print("\nSaving final Finnish translations...")
    with open("/home/shirmash/research_methods/translations/he_fi_dict_llama3_pivot.pkl", "wb") as f:
        pickle.dump(finnish_dict, f)
    
    print("Finnish translation completed!")
    print(f"Total items processed: {len(finnish_dict)}")
